...
按七种预设过了一遍谁真的要地图。**地图是首层模块的预设只有 TRPG(第 3 位)与
设定集(第 2 位)** —— 小说和剧本里它都在「其他」区,也就是说此前一直在为
「地图属于二线模块」的题材做设计。

1. **疆域尺度**(设定集 / 奇幻 / 纪实历史):`MapRegion` 早就能绑实体(哪块地归哪个阵营)、
   绑时间点区间(疆域随时代变迁)、分图层。**能力齐了,缺的是没人知道**
2. **平面图尺度**(推理现场 / 舞台平面图 / TRPG 遭遇图):三类**共用一套构件**,
   不是推理专用。此前完全空缺
3. **战术网格**(TRPG 刚需,另两类也用得上):`MapDoc` 里没有,真缺口
4. **互动叙事的「地图」其实是关卡连接图**,那是流程模块的事 —— 明确不做

**露出(第 1 类)**:`TimeRangeFields` 在项目还没有时间点时**直接 `return null`**,
「这东西可以在某个时代出现、另一个时代消失」就永远不会被发现 —— 而**新项目必然
没有时间点,等于对每个新用户都不存在**。这类「前置数据没有就假装功能不存在」的写法
是能力被埋掉的头号原因,看到就该改成给引导。时间线滤镜、区域按钮 tooltip 同批处理。

**构件(第 2 类)**:新增 `door` / `window` 两个 shape 类型(开合弧与双线,折线画不出来);
**房间 / 墙 / 家具只是预设了粗细与填充的 rect / polyline,不占新类型**。
做成 `SHAPE_TOOLS` 一张表,加工具只改表,不动 click / drag / 渲染三处分支。
绘制工具**连放**(画完保持,Esc 退出)。门的朝向靠对调两个端点,不加字段 ——
自查用的图,朝向对不对作者一眼看得出来,加下拉是让画图变成填表。

**网格(第 3 类)**:只做方格。六角服务的是陆地旅行那种疆域尺度,而网格是为平面图做的。

- **纵向格距 = 边长 × 宽高比**。画布是 `preserveAspectRatio="none"` 的拉伸坐标系,
  不乘的话格子在屏幕上是长方形、量距离全错
- **真正让画图变快的是吸附而不是网格本身**:房间四角落在格上,墙才自然接头
- 换算(「1 格 = 1 米」)只作图例,应用不参与计算。图例走 HTML 不走 svg `<text>` ——
  拉伸坐标系里的文字会变形(**既有的「文字」形状就有这个毛病**,本批没动)

**对齐外部底图**:自带网格的底图需要**边长 + 两个偏移共三个自由度**,少了偏移永远对不上
(底图有页边距,第一条格线不在 x=0)。所以 `grid` 存 `size/offsetX/offsetY` 而不是
「横向格数」—— 格边长几乎不会正好等分图宽。三条路径:拖 N 格校准、按底图每格像素
换算(`imageWidth` 已存,零误差)、手动微调。**校准时刻意不吸附** —— 要对齐的正是
底图自己的格线,吸到旧网格上就白拖了。

**几何抽到 `src/mapGrid.ts`**(`gridCell` / `snapToGrid` / `gridFromDrag`):忘乘宽高比、
负数取模、从一次拖拽反推三个自由度,全是算错了也不报错、只能靠肉眼看图的东西,
留在组件里测不了。

### 走查 B 级收尾

- **`BrainNote.usedIn`**:转成场景 / 大纲行 / 资料卡 / 实体自动记一笔,也可手动标。
  只增不减、不去重 —— 要回答的是「哪些还没用过」。**记录必须直接写 store**:
  转换完立刻跳走,画布卸载会把 350ms 防抖落盘 `clearTimeout` 掉,第一版就是这么丢的
  (测试全绿,只有实点才看得见)
- **`ForeshadowRef.rowId`**:疑点与倒着规划的伏笔都是构思阶段列的,那时一个场景都没写。
  `docId` 改可选,两个锚点都失效才丢弃。**删大纲行要显式解绑** ——
  `commit` 不跑 `normalizeProject`(只有加载时跑),否则台账挂一串「(已删除)」直到重开项目
- **`ResearchCard.verified`**:待查 / 已证实 / 存疑 / 已推翻,md frontmatter 往返。
  筛选下拉只在真有卡片标过之后才出现
- 便签颜色改按张数轮转(随机色等于替用户先洗一遍牌);「写」阶段图标 doc → pencil
  (doc / grid / cards 三个都是矩形加线条,15px 并排分不出来)

**验证方法上的两条**:`@container (max-width: 300px)` **是闭区间**,把默认宽度改到
正好 300 时规则仍然命中(A3 第一版就栽在这);**被超时打断的浏览器脚本产生的数据不能信** ——
连画三个构件的脚本超时后,数据里出现「用房间画出来的却是 window 类型」,
清空重来、一次只画一个,三个类型全对。

## 最近变更(v0.66.0 推理流程走查:疑点台账 + 五处修复)

把应用当「本格推理短篇 · 新系列」的工具从头走一遍(撒便签 → 查考据 → 设计主角 →
画现场图 → 搭大纲 → 写正文 → 核对逻辑 → 盘伏笔 → 比改写 → 跑校对)撞出来的一批。

**新增**

- **`Foreshadow.kind`(`setup` | `doubt`)**:伏笔与疑点**共用同一套两锚点结构与状态推导**,
  差别只在措辞(埋设/回收 vs 提出/排除)。「排除了所有不合理才是真相」这一步此前无处可记。
  台账按类型分页 —— 混在一张表里表头没法同时对两边。**旧数据无 kind 一律按伏笔算**,
  `normalizeProject` 把非法值删成 undefined。「缺提出」与「缺埋设」同理是公平性违规探测器。
  写作面板的待办数字只数伏笔:「埋了没回收」和「提了没排除」是两件事
- **风暴便签「转为资料卡」「转为实体」**:此前只有场景与大纲行,而推理写作的顺序是
  先查证、先立人物才轮到写场景。实体一律建成 `concept` 交由用户改 —— 便签文字看不出类别

**修复(每条都是走查中撞到的)**

- **新建后不聚焦命名框**(风暴便签 / 资料卡 / 地图标记)。正文 `setFocusBlockId` 与
  设定集 `setPendingNameFocus` 早就是对的。地图标记不命名就是画布上一个无名黑点。
  风暴板走模块级 `pendingNoteFocus` + Sticky 挂载时自取 —— 要跨 Canvas 与 Sticky 传一次性信号
- **`--pane-nav` 默认 260px < 容器查询阈值 300px**。「拖窄了才收成图标」从未生效,
  所有人从第一次打开就看到无字图标,而空状态引导按名字指路。**改 300 仍然不行 ——
  `@container (max-width: 300px)` 是闭区间**,正好 300 也命中;最终 320。
  fallback 在 CSS 与 `PaneHandle.tsx` **两处**,必须同步
- **`proofreading.ts` 半角检查漏引号/句点/括号**。直引号单列一条规则,不走「前后有汉字」
  判定 —— 对白收尾引号前面往往是「？」而不是汉字,那条判定会整个漏掉;改判「块内有汉字」,
  开合按块内奇偶推断。`.` 跳过与另一个点相邻的情况(省略号交给「连续标点」)
- **`planning.ts:81` 提及只按全名匹配**。正文写「周师傅」而实体叫「周敬堂」即为 0。
  中文小说里全名出现反而少见。改为 name + aliases,`proofreading.ts:137` 早有先例。
  同块出现全名与别名只算一次(mentions 记的是块数)
- **地图空状态推荐奇幻地图工具**(Inkarnate / Azgaar / Wonderdraft 与「阵营领地」),
  而实际进去就能画室内平面图。功能比文案强

**走查方法**:全新空槽位、真实点 UI、不灌 localStorage —— 卡顿和绕路本身就是要找的东西。
产出 10 便签 / 1 考据卡 / 5 实体 / 6 标记 / 5×4 大纲 / 629 字正文 / 5 伏笔 / 1 快照。
**连线与拖拽仍无法在无头环境验证**(ResizeObserver 不触发 → React Flow 给未测量节点挂
`visibility:hidden` → 元素不可聚焦、连线不渲染)。A1 的风暴便签聚焦就是这么"失败"的 ——
挂探针确认 effect 触发、`focus()` 已调用、仅因 `visibility:hidden` 无效,手动放开即成功,
最后由用户在本机确认。**看到「明明写了却没生效」先查这条,别急着当回归。**

**仍未修(下一批候选)**:伏笔锚不到大纲行(只能挂已存在的场景文档,构思阶段挂不上)、
便签颜色随机(资料卡与实体都是确定性轮转)、地图标记不能连放、资料卡缺证实状态与年代适用、
便签与资料卡没有「用没用到」(实体有「出现于 N」、灵感库有 `usedIn`)、
**系列跨案复用无位置**(实体只能靠项目内文件夹分组,跨项目只有灵感库且只装纯文字)。

## 最近变更(v0.65.1 新建对象必须可见)

用户反馈「风暴板加不上新便签了」。**并不是加不上** —— 便签建出来了,只是落在屏幕外,
点几次攒了三张空白的。查下来这是**一类**问题,五处同形:

- **`nextNotePosition(notes, origin?)`**:原来固定从 `NOTE_ORIGIN` (80,80) 起扫格点。
  板子铺开后左上角那几格往往还空着,新便签就落在视口外。改为调用方传当前视口左上角
  (`screenToFlowPosition` 容器矩形 + 40px 内缩);不传仍回落到原点。
  **`FlowEditor.addNode` 早就是对的**(`screenToFlowPosition(容器中心)`),风暴板当时没抄
- **资料 / 正文 / 设定集 / 资源**:新建对象继承了当前**分类**筛选(所以看得见),
  但 `query` / `tagFilter` / `revFilter` / `kindFilter` 没清 —— 新对象没有标签、
  标题不含搜索词、`revision === undefined`,当场被筛没。四处 create 里补上清理。
  这四个文件里跨模块跳转的 effect 本来就在选中前清筛选,新建走同一条路

**判断依据**:分类不清是对的(新对象本就建进当前分类);清的是那些新对象**必然不满足**的条件。

**这类 bug 测试挡不住** —— 数据写进去了,断言全绿,只有在浏览器里带着筛选点一次「新建」
才看得见。四处都实测过:筛选条件设成会挡住新对象的值 → 新建 → 确认筛选被清 + 新对象出现在列表里

地图(标记落在点击处)、关系图(无新建入口)、流程编辑器均无此问题。
...
- **`panes.ts`**:分屏两条规则 —— `jumpTargetPane`(目标已在副面板则不动主面板)与 `openInPrimary`(撞上副面板的模块就两侧对调)。**同模块不得双开**:两个实例各自维护选中项,跳转落在哪一侧是竞态。主面板切换收敛到 `openPrimary` 一个入口,副面板标签栏禁用主面板正在显示的模块
- **`workspace.defaultSecondaryTab` / `stageHomeTab`**:分屏默认配对与阶段落点都按预设算,不再写死 documents↔flow
- **`moduleName.useModuleName`**:界面文案提到别的模块时名字跟预设走 —— 小说预设下写死的「实体」「文档」会把人指向不存在的按钮
- **`components/Q.tsx`**:UI 文案里 `「」` 圈出的界面元素名着色(`--ink-quote`)。**两次踩坑**:① 第一版改的是 `RichText`,给正文内容上色了 —— 目标错了,已 revert;② 脚本替换时模板字符串里的 `「${name}」` 被误替换成 `<Q>` 标签,**tsc 与全部测试都是绿的**(它只是个合法字符串),靠 grep `message:` / 反引号才发现。第三版加字符串守卫,但守卫过严又漏掉三元运算返回的可见文案,再补一轮
- WritingDashboard 空态、规划各视图空态引导、顶栏工具菜单分组一并整理
...
- `WritingStage` 增 `'codex'`(标签「设」):设定集 / 地图 / 时间线 / 规划 / 资料提到最前
- `primaryTabsFor` 的重排规则改为查 `STAGE_FIRST` 表,plan 与 codex 共用同一段逻辑
- **阶段开关从「散文型预设」放宽到「除通用外全部预设」**(`hasStages`)—— 互动叙事与 TRPG 也需要查设定和理结构;`DocumentView` 的阶段联动 effect 同步换成 `hasStages`
- 「设定集」预设保留但改了说明:它是「整部作品就是设定集」的场景,不是「写小说时要查设定」
- `workspace.test.ts` 加一条:**每个预设的 codex 阶段集合 = write 集合 ∪ 五个设定模块**,且通用预设不受阶段影响

v0.62.2 / v0.62.3 是两个可发现性修复(均由实机使用发现):

- **新建项目不问预设**:预设只在首次欢迎页问一次,`newSlot` 路径完全不问,而改预设的入口标题叫「工作区布局」,用户找不到。改为「新建空白项目…」两步选预设 + 标题改「工作区预设(随时可改)」
- **欢迎页选了布局可能不生效**:原先只有点两张卡片才写 `workspacePreset`,点「跳过引导」/ Esc / 点背景会静默丢弃。改为 `pick()` 统一出口,三条退出路径都写入,并补「已选「X」布局」确认行与「使用「X」布局并关闭」按钮
