...
- 最近验证(2026-07-17,含当前未提交改动):`npm test` 179 项通过、`npm run build` 通过、`cargo test --lib` 2 项通过;体检筛选 / 跳转、路径报告、组合查询及保存 / 更新 / 套用 / 重命名 / 删除已在浏览器实际点验

### 当前执行顺序

1. **v0.23.1 文件夹模式稳定性小批**:✅ 开发、验证与独立提交完成,等待发布
...
下一主批为 R10-A。AI 修复与自然语言查询必须消费已落地的统一问题模型和保存查询。
