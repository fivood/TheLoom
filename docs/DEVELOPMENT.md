...
- 文件夹绑定记录保存在对应项目槽位元数据中；项目切换会先冲刷当前槽位的本地镜像与绑定目录,再读取目标槽位自己的绑定目录。旧版全局 `theloom-folder` 记录在首次启动时迁移到当前槽位
- 桌面端新建项目默认先选目录；槽位 `folderOnly: true` 表示用户已在完整落盘后清除浏览器镜像。此模式后续编辑只写绑定目录；解绑前必须重新写入本地镜像。浏览器资源缓存只删除已落盘且未被其他槽位 / 快照引用的 hash
