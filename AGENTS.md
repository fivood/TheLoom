...
## 当前进度与下一计划(2026-07-17)

- 已发布:`v0.24.0 / R10 高级体检与查询`
- 已完成:R0-R9,包括统一叙事单元、长篇正文工作台、文档—流程双视图、小说规划与修订、AI 轻量抽取与完整小说导入、脚本 AST、路径测试、资源原文件闭环、Excel / Final Draft 互通和通用引擎包
- 当前基线:`v0.24.0`;包含 v0.23.1 文件夹模式稳定性修复与完整 R10
- 已完成主批:`v0.24.0 / R10 高级体检与查询`
  1. ✅ 统一 audit / 脚本诊断 / 路径测试的问题模型、严重级别与跨模块定位
  2. ✅ 增加无效引用、结构循环、时间冲突和角色一致性规则
  3. ✅ 增加按对象类型 / 文件夹 / 属性 / 标签 / 状态 / 引用关系的组合查询
  4. ✅ 支持保存、更新、套用、重命名、删除查询,迁移、文件夹往返、5000 对象性能与交互回归通过
- 下一主批:`v0.25.0 / R10-A AI 深度助手`;详细设计见 `docs/R10A_AI_ASSISTANT.md`
  1. R10-A1 安全内核:Provider 能力 / 结构化输出 / 取消 / usage;显式上下文;结构化提案;dry-run 验证
  2. R10-A2 只读助手:带引用问答;自然语言 → 本地 `ProjectQuery` → 可选保存
  3. R10-A3 脚本与问题修复:消费 `ProjectIssue`,通过 AST / 类型 / audit / path 后才可应用
  4. R10-A4 叙事分析:路径、人物一致性、POV / 时间 / 弧线、伏笔与节奏,事实结论必须有对象依据
  5. R10-A5 完整互动项目生成:计划 → 候选 → ImportPreview → 脚本 / 引用 / 路径验收
  6. R10-A6 性能、隐私、取消 / 离线恢复、浏览器 / Tauri 与发布回归
- 模型只拥有白名单只读工具,永远不直接操作 store;修改必须是带基线指纹的白名单提案,在项目副本上验证、用户预览确认后单次 `commit`
- 当前开发:R10-A1 Provider 结构化输出入口已完成首批实现(OpenAI JSON Schema / Anthropic 强制 tool / Ollama format / 兼容降级 / 本地校验 / 取消与错误分类)
- 最近验证(含当前未提交改动):`npm test` 188 项通过,`npm run build` 通过,`cargo test --lib` 2 项通过;体检筛选 / 跳转、路径报告、组合查询及保存查询全生命周期已在浏览器实际点验
...
| **资源** | assets/ | `Asset`(图片/音频/视频/文件,缩略图 + SHA-256 内容寻址原文件,桌面文件夹 / 网页 IndexedDB 双存储) |
...
**明确暂缓**
- 矢量地点编辑、跨模块多面板 / 多窗口
- Localization 项目内容本地化与 VO 工作流
- 演出 / 流程节点内直接播放挂接的音视频资源(资源库内播放与原文件闭环已完成)
