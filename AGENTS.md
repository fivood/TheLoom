...
- 当前工作区:`v0.23.1` 文件夹模式稳定性发布候选已完成,尚未提交 / 打标签 / 发布;不要把后续 R10 改动混入该批
...
- 最近验证(含当前未提交改动):`npm test` 167 项通过,`npm run build` 通过,`cargo test --lib` 2 项通过;项目菜单与弹窗已在浏览器实际点验,控制台无错误
