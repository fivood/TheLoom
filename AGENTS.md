...
- 下一主批:`v0.25.0 / R10-A AI 深度助手`;AI 修复和自然语言查询必须消费 R10 的结构化结果,不得另建绕过校验的写入路径
- 最近验证(含当前未提交改动):`npm test` 179 项通过,`npm run build` 通过,`cargo test --lib` 2 项通过;体检筛选 / 跳转、路径报告、组合查询及保存查询全生命周期已在浏览器实际点验
