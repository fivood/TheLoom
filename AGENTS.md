...
- 已发布:`v0.23.0 / R9 通用游戏引擎导出`
- 已完成:R0-R9,包括统一叙事单元、长篇正文工作台、文档—流程双视图、小说规划与修订、AI 轻量抽取与完整小说导入、脚本 AST、路径测试、资源原文件闭环、Excel / Final Draft 互通和通用引擎包
- 当前工作区:`v0.23.1` 文件夹模式稳定性已提交(尚未推送 / 打标签 / 发布);`R10-1` 与 `R10-2` 已完成开发与验证
- 下一主批:`v0.24.0 / R10 高级体检与查询`
  1. ✅ 统一 audit / 脚本诊断 / 路径测试的问题模型、严重级别与跨模块定位
  2. ✅ 增加无效引用、结构循环、时间冲突和角色一致性规则
  3. 增加按对象类型 / 文件夹 / 属性 / 标签 / 状态 / 引用关系的组合查询
  4. 支持保存、编辑、删除查询,补迁移、性能与交互回归
- R10 验收后再进入 R10-A AI 深度助手;AI 修复和自然语言查询必须消费 R10 的结构化结果,不得另建绕过校验的写入路径
- 最近验证(含当前未提交改动):`npm test` 171 项通过,`npm run build` 通过,`cargo test --lib` 2 项通过;项目菜单、弹窗、严重级别 / 范围筛选、问题跳转与路径报告已在浏览器实际点验,控制台无错误
