...
## 当前进度与下一计划(2026-08-25)

- 已发布:`v0.55.1 / PWA 与移动端 Obsidian 级纯净写作与灵感流重构`
- 已发布:`v0.55.0 / 地图 NavigatorTree 树、风暴便签快捷键与实体表格交互升级`
- 已完成:`v0.54.4 / PWA 与移动端体验修复`、`v0.54.0 跨项目灵感库 + 开篇动线 + 云房间下线`、`v0.54.1/v0.54.2 UX 走查 A/B/C 级修复`
- 已完成:`R11-1 命名模板对象与安全实例迁移`
- 已完成:`R11-2 资源 / 文档 / 地图模板与统一模板管理器`
- 已完成:`R11-W 完整写作工作台`;详细设计见 `docs/R11W_WRITING_WORKBENCH.md`
  1. ✅ 写作 / 结构 / 连续稿三视图与专注模式
  2. ✅ 普通正文块、键盘连续写作、斜杠菜单、角色搜索与就地创建
  3. ✅ 文档—流程稳定关联、选择性流程角色与显式结构更新
  4. ✅ Obsidian 可读正文权威往返、旧围栏迁移与卷 / 章真实子目录
- 已完成:`R11-3 五模块收藏夹 + 单次撤销的批量编辑`
- 已完成:`R13 首批专业导入导出`
  1. ✅ TXT / Markdown 长稿编码识别、卷章场景解析与统一预检
  2. ✅ EPUB 按 OPF / spine 顺序提取 XHTML 并拆分章节
  3. ✅ DOCX 按标题样式还原项目 / 卷 / 章 / 场景层级
...
- 当前基线:网页 / 桌面 `v0.55.1`
- 最近验证:`npm test` 600 项通过,`npm run build` 通过;Playwright 实机自动化覆盖全 11 模块与全移动视口(38/38 实机项通过);`cargo test --lib` 4 项通过
...
- **地图统一 NavigatorTree 树**:地图列表从旧版 `side-list` 升级为 `NavigatorTree`(`types.ts` `FolderModule` 加入 `'map'`),支持多级文件夹分组、拖拽排序、批量归档、双击重命名与属性栏文件夹归属
- **风暴便签快捷键增强**:支持 `Ctrl+C` 复制便签与连线、`Ctrl+V` 偏移粘贴、`Ctrl+D` 原地生成副本;输入框/文本域焦点智能避让
- **实体表格视图引用字段交互式选择**:移除只读提示,接入 `<EntityRefEditor />` 单选下拉与多选芯片组,即选即存
- **AutoSync 空指针防护**:加固 Zustand 订阅回调中 `prev.project` 的可选链保护
- **全量实机验证**:Playwright 覆盖 11 模块、8 规划视图、全局搜索、分屏与移动壳(38/38 实机项通过,600 单元测试全部通过)
