...
/** 变量 / 实体快捷插入(校验与高亮由 ScriptInput 内联提供) */
function ScriptHints({ onInsert }: { onInsert: (name: string) => void }) {
        ...
        {selectedNode ? (
          <>
            ...
            <div className="field">
              ...
              {selectedNode.type === 'dialogue' || selectedNode.type === 'fragment' || selectedNode.type === 'jump' ? (
                ...
                <ScriptInput
                  value={selectedNode.data.text}
                  onChange={(v) => patchSelectedNode({ text: v })}
                  mode={selectedNode.type === 'condition' ? 'condition' : 'instruction'}
                  rows={selectedNode.type === 'instruction' ? 3 : 2}
                  placeholder={selectedNode.type === 'condition' ? '如:semelvie.trust > 5 && seen("intro")' : '如:took_book = true; favor += 1'}
                />
              ) : (
            ...
            <TechNameField
              ...
              onRename={(oldName, newName) => useLoom.getState().renameSeenTarget(oldName, newName)}
            />
