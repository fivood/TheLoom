...
import { WORKSPACE_PRIMARY_TABS, defaultSecondaryTab, presetHomeTab, primaryTabsFor, stageHomeTab, workspaceTabLabel } from './workspace';
...
describe('R17-4 项目工作区预设', () => {
  ...
  it('分屏默认配对:副面板不与主面板同模块,写正文时配设定集 / 流程', () => {
    for (const preset of WORKSPACE_PRESETS) {
      for (const primary of WORKSPACE_PRIMARY_TABS[preset]) {
        expect(defaultSecondaryTab(preset, primary)).not.toBe(primary);
      }
    }
    expect(defaultSecondaryTab('novel', 'documents')).toBe('entities');
    expect(defaultSecondaryTab('screenplay', 'documents')).toBe('entities');
    expect(defaultSecondaryTab('interactive', 'documents')).toBe('flow');
    expect(defaultSecondaryTab('novel', 'outline')).toBe('documents');
    expect(defaultSecondaryTab('codex', 'entities')).toBe('documents');
  });
});
